...
EXPECTED_APK="Blee-2.0.0-mesh-v2-debug.apk"

# Prefer the already-installed Homebrew JDK before the legacy base builder tries
# its network fallback. This keeps repeat builds deterministic on Apple Silicon.
...
replacements = {
    'APP_NAME="Blee-1.1.1-storage-fixed-debug.apk"': 'APP_NAME="Blee-2.0.0-mesh-v2-debug.apk"',
    "g=re.sub(r'versionCode\\s+\\d+', 'versionCode 4', g)": "g=re.sub(r'versionCode\\s+\\d+', 'versionCode 8', g)",
    "g=re.sub(r'versionName\\s+\"[^\"]+\"', 'versionName \"1.1.1\"', g)": "g=re.sub(r'versionName\\s+\"[^\"]+\"', 'versionName \"2.0.0\"', g)",
    "print('Blee 1.1.1 launcher identity and version verified.')": "print('Blee 2.0.0 launcher identity and version verified.')",
}
for old, new in replacements.items():
    if old not in text:
        raise SystemExit(f'Blee 2.0 builder could not locate expected base-build marker: {old}')
    text = text.replace(old, new, 1)

anchor = '''echo "Applying Blee 1.1 wallet recovery + custom network support..."\npython3 scripts/apply-blee-1.1.py\n'''
addition = anchor + '''\necho "Applying Blee 1.3 identity, QR, discovery and profile UX..."\npython3 scripts/apply-blee-professional.py\n\necho "Applying Blee 1.4 brand and ledger timestamp patch..."\npython3 scripts/apply-blee-1.4.py\npython3 scripts/apply-blee-1.4-finalize.py\n\necho "Applying Blee 2.0 Mesh v2 event ledger + runtime..."\npython3 scripts/apply-blee-mesh-v2.py\n'''
if anchor not in text:
    raise SystemExit('Blee 2.0 builder could not locate Blee 1.1 overlay stage')
...
echo "============================================================"
echo "Building Blee 2.0 — Mesh v2"
echo "============================================================"
echo "Architecture: ARCHITECTURE.md"
bash "$INNER_BUILDER"

APK="$ROOT/dist/$EXPECTED_APK"
[ -f "$APK" ] || { echo "ERROR: Blee 2.0 APK was not produced: $APK"; exit 1; }
...
echo
echo "Blee 2.0 Mesh v2 build complete:"
echo "$APK"
