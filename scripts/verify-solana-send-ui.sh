...
# SOL send presentation may change, but these boundaries may not.
...
grep -q 'data-blee-send-ui="approved-v1"' "$APP" || fail "approved Send UI marker missing"
grep -q 'app.solana.offlineReady' "$APP" || fail "SOL send does not fail closed without a prepared nonce"
grep -q 'app.meshStarted' "$APP" || fail "SOL send does not require Blee nearby transport"
grep -q 'recipientEvm: payAddress' "$APP" || fail "SOL recipient is not passed as Blee identity"
...
grep -q "TokenLogo asset={sol ? 'sol' : 'usdc'}" "$APP" || fail "Send asset selector is not visually rail-bound"
grep -q 'Fees are paid in SOL.' "$APP" || fail "SOL fee ownership note missing"
...
if grep -Eq 'recipientSolana[[:space:]]*:|sendSignedSolanaTransaction|createSolanaRpc|https://api[.](mainnet-beta|devnet|testnet)[.]solana[.]com|helius-rpc[.]com' "$APP"; then
  fail "SOL send UI must not accept a raw recipient Solana address or bypass coordinator/gateway boundary"
fi
