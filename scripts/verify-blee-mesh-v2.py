...
def main() -> None:
    ...
    require(
        ...
        (
            "BLEE_STORE_MESH_V2_ATOMIC_SIGNING_V2", "readLong(PluginCall call, String key)",
            "signing_intents", "reserveSigningIntent", "finalizeSigningIntent", "abortSigningIntent",
            "SIGNATURE_BUNDLE_PERSISTED", "database.beginTransaction()", "state='SIGNING'",
            "PERSISTED", "setWriteAheadLoggingEnabled(true)",
        ),
    )
    atomic = require(
        ROOT / "src/lib/atomicSigning.ts",
        (
            "createAtomicAuthorization", "refreshAtomicSigningProfile", "reserveSigningIntent",
            "finalizeSigningIntent", "abortSigningIntent", "SIGNING_SESSION_ID", "SENDER_FUNDED_RAW_TX",
            "bundleHash", "signTypedData", "signTransaction", "BLEE_ATOMIC_BRIDGE_STRING_NUMERICS",
            "chainId: String(arcTestnet.id)",
        ),
    )
    payments = require(
        ROOT / "src/lib/payments.ts",
        ("createAtomicAuthorization", "refreshAtomicSigningProfile", "sendRawTransaction", "verifyAuthorization"),
    )
    recovery = require(
        ROOT / "src/lib/walletRecovery.ts",
        ("passphrase.length < 8", "at least 8 characters", "PBKDF2", "AES-GCM"),
    )
    vault = require(ROOT / "src/lib/vault.ts", ("createVault", "passphrase.length < 8", "at least 8 characters"))
    ...
    css = require(
        ...
        ("BLEE_UI_2_4", "grid-template-columns: repeat(4, 1fr)", ".blee-phone.with-nav", ".screen-transition", "safe-area-inset-bottom"),
    )
    network = require(
        ROOT / "src/lib/networkConfig.ts",
        ("Arc Testnet", "chainId: 5042002", "USDC", "0x3600000000000000000000000000000000000000", "ARC_TESTNET", "Custom settlement networks are not available in Blee"),
    )
    advanced = require(ROOT / "src/components/BleeAdvancedSettings.tsx", ("getActiveNetwork", "settlement network"))
    runtime = require(
        ROOT / "src/components/BleeRuntime.tsx",
        ("registerPlugin<MeshPlugin>('BleeMesh')", "ledgerChanged", "pendingEnvelopes", "verifyAuthorization", "acceptEnvelope", "refreshSenderFundedSettlementProfile"),
    )
    ...
    banned_ui = (
        ...
        "Payments that keep moving", "brand-orbit", "hero-ring", "signal-ring", "radar-ring",
        "brand-monument", "People around you",
    )
    found_ui = [t for t in banned_ui if t.lower() in app.lower()]
    if found_ui:
        raise SystemExit(f"VERIFY ERROR: legacy presentation survived rebuild: {found_ui}")
    if not css.lstrip().startswith("/* BLEE_UI_2_4"):
        raise SystemExit("VERIFY ERROR: globals.css is not the 2.4 replacement stylesheet")

    if "saveNetwork" not in network or "throw new Error" not in network:
        raise SystemExit("VERIFY ERROR: Arc-only compatibility API is not locked")
    if any(t in advanced for t in ("saveNetwork", "deleteNetwork", "setActiveNetwork", "Custom network")):
    ...
    for name, markers in {
        ...
        "BleeMeshService.java": (
            ...
            "attemptSenderFundedSettlement", "eth_sendRawTransaction", "SENDER_FUNDED_RAW_TX",
            "TRUSTED_CHAIN_ID = 5042002L", "settlementRetryAfter.put(paymentId, Long.MAX_VALUE)",
            "BLEE_BLUETOOTH_DISCOVERY_V2_2", "ADVERTISE_MODE_LOW_LATENCY", "ADVERTISE_TX_POWER_HIGH",
            "SCAN_MODE_LOW_LATENCY", "MATCH_MODE_AGGRESSIVE", "onScanFailed", "CONNECTION_PRIORITY_HIGH", "PHY_LE_CODED_MASK",
        ),
        "BleeMeshPlugin.java": ("BleeMesh", "ledgerChanged", "pendingEnvelopes", "acceptEnvelope", "broadcastMode", "senderPaysGas", "relayPaysGas"),
    ...
    require(
        ...
        (
            ...
            "android.permission.POST_NOTIFICATIONS", "android.permission.FOREGROUND_SERVICE_CONNECTED_DEVICE",
            "android.permission.RECEIVE_BOOT_COMPLETED", "BleeMeshService", "BleeBootReceiver",
        ),
    )

    generated = "\n".join(p.read_text(errors="replace") for p in activity.parent.glob("Blee*.java"))
    ...
    banned_backend = ("attemptSponsoredSettlement", "SPONSORED_AUTO_RELAY", "mesh.relay.endpoint", "configureRelay", "NEXT_PUBLIC_BLEE_RELAY_ENDPOINT")
    found_backend = [m for m in banned_backend if m in generated or m in web]
    if found_backend:
    ...
    print("- replacement UI + four-destination navigation")
    print("- functional 8-character create/import policy")
    print("- Arc Testnet + USDC only")
    print("- explicit logout-only session policy")
    print("- crash-atomic signing + sender-funded relay")
    print("- BLE low-latency/high-power discovery hardening")
    print("============================================================")
