...
def fail(message: str) -> None:
    raise SystemExit(f"Blee contacts React v4: {message}")


def ensure_register_plugin(text: str) -> str:
    if re.search(r"import\s*\{[^}]*\bregisterPlugin\b[^}]*\}\s*from\s*['\"]@capacitor/core['\"]", text, re.S):
        return text
...
def patch_app() -> None:
    ...
    text = APP.read_text()
    if "BLEE_CONTACTS_REACT_V4" in text:
        verify_app(text)
        return

    # Guard against accidentally stacking the abandoned v3 patch on a reused tree.
    if "BLEE_CONTACTS_REACT_V3" in text:
        fail("v3 React contacts already exist; canonical full build must start from clean materialized UI")
...
// Contacts are part of the real React Activity screen. Durable records live in
// Blee's native SQLite store; no MutationObserver or DOM injection is used.
type BleeContact = {
  wallet: string;
  displayName?: string;
  avatar?: string;
...
'''
    ...
    short_pattern = re.compile(
        r"function short\(value\?: string \| null\) \{\s*if \(!value\) return '—';\s*return `\$\{value\.slice\(0, 6\)\}…\$\{value\.slice\(-4\)\}`;\s*\}",
        re.S,
    )
    short_replacement = """function short(value?: string | null) {
  if (!value) return '—';
  const stable = canonicalWallet(value);
  return `${stable.slice(0, 6)}…${stable.slice(-4)}`;
}"""
    text, short_hits = short_pattern.subn(short_replacement, text, count=1)
    if short_hits != 1:
        fail("wallet short formatter anchor missing")
...
'''
    ...
    filter_pattern = re.compile(
        r"  const filteredPayments = useMemo\(\(\) => visiblePayments\.filter\(\(row\) => activityFilter === 'all' \|\| \(activityFilter === 'sent' \? row\.direction === 'out' : row\.direction === 'in'\)\), \[visiblePayments, activityFilter\]\);"
    )
    filter_replacement = """  const filteredPayments = useMemo(() => visiblePayments.filter((row) => {
    ...
    text, filter_hits = filter_pattern.subn(filter_replacement, text, count=1)
    if filter_hits != 1:
...
'''
    ...
    activity_replacement = r'''  const renderActivity = () => (
    <div className="screen-content"><ScreenHeader title="Activity" trailing={pendingPayments.length ? <span className="count-badge">{pendingPayments.length}</span> : null}/>
      ...
      <button className={`activity-contact-filter ${selectedContact ? 'active' : ''}`} onClick={() => { void refreshContacts(); setContactPanel('filter'); }}>
        <span>{selectedContact?.displayName || 'Filter by contact'}</span><span aria-hidden="true">⌄</span>
      </button>
    ...
    text = replace_region(text, "  const renderActivity = () => (", "  const renderActivityDetail = () => {", activity_replacement, "renderActivity")

    detail_start = text.find("  const renderActivityDetail = () => {")
    detail_end = text.find("\n\n  const ", detail_start + 10)
    if detail_start < 0 or detail_end < 0:
        fail("Activity detail boundaries missing")
    detail = text[detail_start:detail_end]

    old_counterparty = "    const counterparty = selectedPayment.counterpartyAlias || app.identityFor(selectedPayment.counterparty)?.alias || short(selectedPayment.counterparty);"
    new_counterparty = """    const savedContact = contactFor(selectedPayment.counterparty);
    ...
    if old_counterparty not in detail:
        fail("Activity detail counterparty anchor missing")
    detail = detail.replace(old_counterparty, new_counterparty, 1)
    detail = detail.replace('src={app.identityFor(selectedPayment.counterparty)?.avatar}', 'src={counterpartyAvatar}', 1)

    tx_anchor = '{selectedPayment.txHash && network.explorerUrl && <a className="secondary-button full"'
    tx_pos = detail.find(tx_anchor)
    if tx_pos < 0:
        fail("Activity detail transaction action anchor missing")
    contact_action = '<button className="secondary-button full contact-detail-action" onClick={() => { const candidate = contactCandidateFor(selectedPayment.counterparty); if (candidate) openContactEditor(candidate); }}>{savedContact ? \'Edit contact\' : \'Save contact\'}</button>'
    detail = detail[:tx_pos] + contact_action + detail[tx_pos:]
    text = text[:detail_start] + detail + text[detail_end:]

    nav_pattern = re.compile(r"(?P<indent>\s*)\{primary && <BottomNav active=\{activeTab\} onChange=\{selectTab\}/>}\s*")
    nav_match = nav_pattern.search(text)
    if not nav_match:
        fail("primary BottomNav root anchor missing")
    nav_text = nav_match.group(0)
    text = text[:nav_match.start()] + nav_text.rstrip() + "\n      {renderContactSheet()}\n" + text[nav_match.end():]

    text = text.replace("Blee 2.5 · Mesh v2", "Blee 2.7 · Mesh v2")
    text = text.replace("Blee 2.6 · Mesh v2", "Blee 2.7 · Mesh v2")
    APP.write_text(text)
    verify_app(text)
...
.activity-contact-filter {
  width: 100%; min-height: 44px; margin: 10px 0 14px; padding: 0 13px;
  display: flex; align-items: center; gap: 9px; border: 1px solid rgba(9,9,9,.10);
  border-radius: 14px; background: rgba(255,255,255,.72); color: #111;
  font: inherit; font-size: 13px; font-weight: 650; text-align: left;
}
.activity-contact-filter > span:first-child { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.react-contact-panel {
  position: absolute; left: 0; right: 0; bottom: 0; max-height: min(78dvh, 680px); overflow: hidden;
  padding: 8px 16px calc(18px + env(safe-area-inset-bottom, 0px)); border-radius: 26px 26px 0 0;
  background: #f8f8f6; box-shadow: 0 -20px 70px rgba(0,0,0,.18);
...
.react-contact-close { font-size: 22px; line-height: 1; }
.react-contact-list { max-height: calc(min(78dvh, 680px) - 70px); overflow-y: auto; display: grid; gap: 7px; padding-bottom: 8px; }
.react-contact-row {
  width: 100%; min-height: 62px; padding: 9px 11px; display: flex; align-items: center; gap: 11px;
  border: 0; border-radius: 17px; background: #fff; color: #111; text-align: left; font: inherit;
}
...
.react-contact-action,.react-contact-check { margin-left: auto; font-weight: 700; }
...
.contact-detail-action { margin-top: 12px; }
'''
    CSS.write_text(text + "\n")
...
def verify_app(text: str | None = None) -> None:
    ...
    if text.count("<BottomNav active={activeTab} onChange={selectTab}/>") != 1:
        fail("bottom navigation is missing or duplicated")


def verify() -> None:
    verify_app()
    css = CSS.read_text()
    for marker in ("BLEE_CONTACTS_REACT_V4", ".activity-contact-filter", ".react-contact-sheet", ".react-contact-panel", ".react-contact-row"):
    ...
    print("============================================================")
    print("VERIFIED: Blee contacts React v4")
    print("- contacts are implemented inside the real Activity React component")
    print("- Home stays a clean Recent Activity surface")
    print("- Activity keeps All / Sent / Received plus one contact filter")
