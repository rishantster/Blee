...
def main() -> None:
    ...
    db = db_path.read_text()
    plugin = plugin_path.read_text()
    for marker in ("BLEE_CONTACT_ACTIVITY_SYNC_V1", "backfillPaymentIdentity(normalized, name, photo)"):
        if marker not in db:
            fail(f"DB missing {marker}")
    for marker in ("BLEE_CONTACT_ACTIVITY_WAKE_V1", "CONTACT_UPDATED"):
        if marker not in plugin:
            fail(f"plugin missing {marker}")
    print("VERIFIED: saved contact names/avatars update Activity immediately without touching signed payment data")
