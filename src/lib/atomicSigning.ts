...
async function createAtomicAuthorizationInner(
  ...
  try {
    const reservation = await BleeStore.reserveSigningIntent({
      ...
      value: value.toString(),
      authorizationNonce,
      expiresAt: String(Number(validBefore) * 1000),
