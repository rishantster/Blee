...
function paymentStatusDetail(row: PaymentRecord) {
  if (paymentAssetSymbol(row) === 'SOL') {
    switch (row.state) {
      ...
      case 'failed': return row.error || 'This SOL delivery could not complete.';
      default: return 'SOL Activity reflects durable nearby delivery only and never claims on-chain settlement.';
    }
  }
  switch (row.state) {
...
export function BleeApp() {
  ...
  // BLEE_SOLANA_ACTIVITY_UI_V1
  // Activity merges read-only SOL transport history with the Arc journal only at
  // presentation time. Arc settlement state and SOL custody state remain separate.
  ...
  const pendingPayments = visiblePayments.filter((row) => paymentAssetSymbol(row) === 'SOL'
    ? row.direction === 'out' && row.state !== 'mesh-delivered' && row.state !== 'failed'
  ...
  const selectTab = (tab: PrimaryTab) => { historyRef.current = []; if (tab === 'home' || tab === 'activity') void app.solanaActivity.refresh(); setScreen(tab); };
  ...
  const renderActivityDetail = () => {
    ...
    return <div className="screen-content" data-blee-activity-detail-asset={symbol.toLowerCase()}><ScreenHeader title="Payment details" onBack={() => { setSelectedPayment(null); goBack('activity'); }}/><div className="detail-hero"><PersonAvatar name={counterparty} src={app.identityFor(selectedPayment.counterparty)?.avatar} size="lg"/><span>{incoming ? 'Received from' : 'Sent to'} {counterparty}</span><h1>{incoming ? '+' : '−'}{formatAmount(selectedPayment.amount, isSol ? 9 : 2)} {symbol}</h1><small>{networkLabel} · {formatTimestamp(selectedPayment.createdAt)}</small></div>{isSol ? <section className="timeline-card" data-blee-solana-activity-timeline="delivery-only">{incoming ? <div className="timeline-row done"><span/><div><strong>Received nearby</strong><small>{formatTimestamp(selectedPayment.durablyReceivedAt || selectedPayment.createdAt)}</small></div></div> : <><div className="timeline-row done"><span/><div><strong>Signed and secured</strong><small>{formatTimestamp(selectedPayment.createdAt)}</small></div></div>{selectedPayment.state === 'queued-local' && <div className="timeline-row current"><span/><div><strong>Queued safely</strong><small>Stored locally for exact-byte retry over Blee Mesh.</small></div></div>}{selectedPayment.state === 'mesh-broadcast' && <div className="timeline-row current"><span/><div><strong>Sending nearby</strong><small>{formatTimestamp(selectedPayment.updatedAt)}</small></div></div>}{selectedPayment.state === 'mesh-delivered' && <div className="timeline-row done"><span/><div><strong>Recipient acknowledged</strong><small>{formatTimestamp(selectedPayment.durablyReceivedAt || selectedPayment.updatedAt)}</small></div></div>}</>}</section> : <section className="timeline-card"><div className="timeline-row done"><span/><div><strong>Created</strong><small>{formatTimestamp(selectedPayment.createdAt)}</small></div></div>{selectedPayment.durablyReceivedAt && <div className="timeline-row done"><span/><div><strong>Delivered nearby</strong><small>{formatTimestamp(selectedPayment.durablyReceivedAt)}</small></div></div>}{selectedPayment.submittedAt && <div className="timeline-row done"><span/><div><strong>Submitted to Arc</strong><small>{formatTimestamp(selectedPayment.submittedAt)}</small></div></div>}{selectedPayment.settledAt && <div className="timeline-row done"><span/><div><strong>Confirmed</strong><small>{formatTimestamp(selectedPayment.settledAt)}</small></div></div>}{(!selectedPayment.settledAt && !(selectedPayment.state === 'submitted' && selectedPayment.submittedAt)) && <div className={`timeline-row ${selectedPayment.state === 'failed' ? 'failed' : 'current'}`}><span/><div><strong>{paymentStatus(selectedPayment)}</strong><small>{paymentStatusDetail(selectedPayment)}</small></div></div>}</section>}<section className="detail-list"><div><span>Counterparty</span><strong>{counterparty}</strong></div><div><span>Network</span><strong>{networkLabel}</strong></div><div><span>Route</span><strong>{selectedPayment.route === 'ble-mesh' ? 'Blee Mesh' : selectedPayment.route === 'arc-direct' ? networkLabel : 'Durable queue'}</strong></div><div><span>Status</span><strong>{paymentStatus(selectedPayment)}</strong></div></section>{isSol && <div className="quiet-note"><Icon name="info"/><span>SOL Activity records durable signing and nearby delivery only. Blee does not claim Solana on-chain submission or confirmation from this history state.</span></div>}{transactionUrl && <a className="secondary-button full" href={transactionUrl} target="_blank" rel="noreferrer">View transaction <Icon name="external"/></a>}</div>;
  };
